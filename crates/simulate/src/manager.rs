...
impl<'a> SimulationManager<'a> {
    ...
    pub fn unpack_execution(&self, execution_result: ExecutionResult) -> Bytes {
        match execution_result {
            ...
            ExecutionResult::Halt { reason, gas_used } => panic!("This call halted for {:?} and used {} gas.", reason, gas_used),
            ExecutionResult::Revert { output, gas_used } => panic!("This call reverted with output {:?} and used {} gas.", output, gas_used),
        }
    }
}
