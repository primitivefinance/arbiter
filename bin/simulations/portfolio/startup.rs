...
fn call_mint(
    ...
    let result = admin.call_contract(
        ...
        contract,
        contract.encode_function("mint", (recast_address(*to), mint_amount))?,
        Uint::from(0),
    );
    assert!(result.is_success());
    println!(
        "Minted {} tokens to Arbitrageur: {:#?}",
        mint_amount,
        result.is_success()
    );
    Ok(())
...
fn mint(
    ...
    vec![arbiter_token_x.clone(), arbiter_token_y.clone()]
        .into_iter()
        .try_for_each(|token| {
            call_mint(&mut manager, &token, &arbitrageur.address())?;
            call_mint(&mut manager, &token, &admin.address())?;
            call_mint(&mut manager, &token, &liquid_exchange_xy.address)
        })?;
